...
% kernel is reshaped eigen-vector of A
%
...
k = permute(kernel,[1,2,4,3]);
k =reshape(k,prod([kSize,nv]),nc);
...
for n=1:size(kernel,4)
    tmp_1=conj(kernel(end:-1:1,end:-1:1,:,n));
    tmp_2=sqrt(imSize(1)*imSize(2));
    KERNEL(:,:,:,n) = (fft2c(zpad(tmp_1*tmp_2, [imSize(1), imSize(2), size(kernel,3)]))); %inv(F)*W*F
end
...
for n=1:prod(imSize)
    ...
    %[C,D] = eig(mtx*mtx');
    [C,D,V] = svd(mtx,'econ');%
    %Bo Li note 2-19-2021
%     tmp_angle=angle(C(1,:));%angle(C(irow,:));
%     ph = repmat(exp(-1i*tmp_angle),[size(C,1),1]);
%     C = v*(C.*ph);
