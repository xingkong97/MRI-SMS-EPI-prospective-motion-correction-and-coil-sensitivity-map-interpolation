...
%         end
    ...
    function varargout = subsref(this, S)
        ...
        if ~this.flagAverageDim(3)
            cIx( 1,:) = this.Lin(ixToRaw) - this.skipLin;%this.skipLin=0
        end
        if ~this.flagAverageDim(4)
            cIx( 2,:) = this.Par(ixToRaw) - this.skipPar;%skipPar=0
        end
    ...
    function out = get.dataSize(this)
        ...
        if this.arg.removeOS
            ix = ismember(this.dataDims, 'Col'); %find the position of 'Col' element in this.dataDims
            out(ix) = this.NCol/2;
        end
