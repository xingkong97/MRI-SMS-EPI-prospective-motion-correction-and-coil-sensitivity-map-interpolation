...
% Author: Philipp Ehses (philipp.ehses@tuebingen.mpg.de), Mar/11/2014
    ...
    for b=1:nbuffers
        ...
        bufname = fread(fid, 20, 'uint8').';%=>char
        ipos= find(bufname(:)==0);
        ipos=ipos(1);%ipos is the '\0', end of the bufname
        bufname = regexp(char(bufname), '^\w*', 'match');
        bufname = bufname{1};
        fseek(fid, -20+ipos, 'cof');%numel(bufname)-9, go back to right after the name string        
