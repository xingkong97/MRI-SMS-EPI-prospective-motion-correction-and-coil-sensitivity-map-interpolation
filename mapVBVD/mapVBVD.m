...
% Bo Li,          1/21/2020  *Add SMS Support. It can read all fully
%                             sampled slice and aliasing slices. For SMS=2,
%                             slice=4, the total slices is 6. 
% 
% 
% Input:
% 
...
for s=1:NScans
    ...
    %Bo Li Add 1-30-2020
    lMultiBandFactor=twix_obj{s}.hdr.MeasYaps.sSliceAcceleration.lMultiBandFactor;
    
    twix_obj{s}.sliceimgSMS   = twix_map_obj(arg,'sliceimgSMS',filename,version,rstraj);
    twix_obj{s}.phasecorSMS   = twix_map_obj(arg,'phasecorSMS',filename,version,rstraj);
    twix_obj{s}.refscanSMS   = twix_map_obj(arg,'refscanSMS',filename,version,rstraj);
    twix_obj{s}.refscanPCSMS   = twix_map_obj(arg,'refscanPCSMS',filename,version,rstraj);
...
%     end
    ...
    %mdh_blob_SMS=mdh_blob;
    [mdh, mask, mdhSMS, maskSMS] = evalMDH( mdh_blob, version, twix_obj{s}); % this is quasi-instant (< 1s) :-)    
    clear mdh_blob
    
    %Bo Li Add 1-21-2019 for SMS
    if lMultiBandFactor>1
        nSlice=twix_obj{s}.hdr.Config.NSlc;
        nTotalSlice=nSlice/lMultiBandFactor+nSlice;
        twix_obj{s}.hdr.Config.NSlc=nSlice/lMultiBandFactor;%nTotalSlice
        twix_obj{s}.hdr.Config.OriNSlc=nSlice;
        rm_row=2:((3+twix_obj{s}.hdr.Config.NLin)*nSlice+1);
        filePosSMS=filePos(rm_row(:));
        filePos(rm_row(:))=[];
    end
    ...
    if lMultiBandFactor>1 
        if arg.bReadImaScan
            clear tmpMdhSMS
            isCurrScan = logical( maskSMS.MDH_IMASCAN);
            for f = fieldnames( mdhSMS ).'
                tmpMdhSMS.(f{1}) = mdhSMS.(f{1})( isCurrScan, : );
            end  
            twix_obj{s}.sliceimgSMS.readMDH( tmpMdhSMS, filePosSMS(isCurrScan) );  
        end
        
        if arg.bReadRefScan  
            clear tmpMdhSMS
            isCurrScan =    ( maskSMS.MDH_PATREFSCAN | maskSMS.MDH_PATREFANDIMASCAN )...
                         & ~( maskSMS.MDH_PHASCOR | maskSMS.MDH_PHASESTABSCAN | ...
                              maskSMS.MDH_REFPHASESTABSCAN | ...
                              maskSMS.MDH_RTFEEDBACK | maskSMS.MDH_HPFEEDBACK);
            for f = fieldnames( mdhSMS ).'
                tmpMdhSMS.(f{1}) = mdhSMS.(f{1})( isCurrScan, : );
            end
            twix_obj{s}.refscanSMS.readMDH(tmpMdhSMS, filePosSMS(isCurrScan) );
        end
        ...
        if arg.bReadPCScan
            ...
            clear tmpMdhSMS
            isCurrScan = maskSMS.MDH_PHASCOR & ( ~maskSMS.MDH_PATREFSCAN | maskSMS.MDH_PATREFANDIMASCAN );
            for f = fieldnames( mdhSMS ).'
                tmpMdhSMS.(f{1}) = mdhSMS.(f{1})( isCurrScan, : );
            end
            twix_obj{s}.phasecorSMS.readMDH( tmpMdhSMS, filePosSMS(isCurrScan) );
            ...
            twix_obj{s}.refscanPCSMS.readMDH( tmpMdhSMS, filePosSMS(isCurrScan) );
        end
    end
    %Add
    ...
    clear tmpMdhSMS  mdh  tmpMdh  filePos  isCurrScan

    %Bo Li added sliceimgSMS to scan 1-31-2020
    for scan = {'sliceimgSMS', 'phasecorSMS','refscanSMS', 'refscanPCSMS', 'image', 'noise', 'phasecor', 'phasestab', ...
...
% http://undocumentedmatlab.com/blog/explicit-multi-threading-in-matlab-part1
    ...
    % ======================================
        bit_0 = uint8(2^0); %1
        bit_5 = uint8(2^5); %32
        mdhStart = 1-byteMDH; %-183
        
        u8_000 = zeros( 3, 1, 'uint8'); % for comparison with data_u8(1:3)

        % 20 fill bytes in VD (21:40)
        evIdx   = uint8(    21  + 20*isVD); % 41 1st byte of evalInfoMask
        dmaIdx  = uint8((29:32) + 20*isVD); %[49,50,51,52] to correct DMA length using NCol and NCha
        if isVD
            dmaOff  = szScanHeader;
    ...
    while true
        ...
        try
            ...
            data_u8 = data_u8( mdhStart+end :  end ); %data_u8(1:184)
        catch exc
        ...
        NCol_NCha = double( typecast( data_u8(dmaIdx), 'uint16' ) );  %512, 20 [ushSamplesInScan  ushUsedChannels]
...
function [mdh,mask,mdhSMS,maskSMS] = evalMDH( mdh_blob, version, twix_obj_s)
    ...
    %Bo Li Add 1-30-2020
    lMultiBandFactor=twix_obj_s.hdr.MeasYaps.sSliceAcceleration.lMultiBandFactor;
    if lMultiBandFactor>1
        nSlice=twix_obj_s.hdr.Config.NSlc;
        %index for removal of SMS full sampling all slice kspace
        rm_row=2:((3+twix_obj_s.hdr.Config.NLin)*nSlice+1);
        data_uint32_SMS=data_uint32(rm_row,:);
        data_uint16_SMS=data_uint16(rm_row,:);
        data_single_SMS=data_single(rm_row,:);
        
        data_uint32(rm_row,:)=[];
        data_uint16(rm_row,:)=[];
        data_single(rm_row,:)=[];
        Nmeas=Nmeas-length(rm_row(:));
        %reorder aliasing slc from 0,1,4,7... to: 0,1,2,3,...
%         data_uint16_tmp= data_uint16(:,5);
%         data_uint16_uniq=unique(data_uint16_tmp);
        
        aulEvalInfoMask = data_uint32(:,6:7);
        evalInfoMask1 = aulEvalInfoMask(:,1);
        mask_tmp.MDH_ACQEND            = min(bitand(evalInfoMask1, 2^0), 1);
        mask_tmp.MDH_RTFEEDBACK        = min(bitand(evalInfoMask1, 2^1), 1);
        mask_tmp.MDH_HPFEEDBACK        = min(bitand(evalInfoMask1, 2^2), 1);
        mask_tmp.MDH_SYNCDATA          = min(bitand(evalInfoMask1, 2^5), 1);
        mask_tmp.MDH_RAWDATACORRECTION = min(bitand(evalInfoMask1, 2^10),1);
        mask_tmp.MDH_REFPHASESTABSCAN  = min(bitand(evalInfoMask1, 2^14),1);
        mask_tmp.MDH_PHASESTABSCAN     = min(bitand(evalInfoMask1, 2^15),1);
        mask_tmp.MDH_SIGNREV           = min(bitand(evalInfoMask1, 2^17),1);
        mask_tmp.MDH_PHASCOR           = min(bitand(evalInfoMask1, 2^21),1);
        mask_tmp.MDH_PATREFSCAN        = min(bitand(evalInfoMask1, 2^22),1);
        mask_tmp.MDH_PATREFANDIMASCAN  = min(bitand(evalInfoMask1, 2^23),1);
        mask_tmp.MDH_REFLECT           = min(bitand(evalInfoMask1, 2^24),1);
        mask_tmp.MDH_NOISEADJSCAN      = min(bitand(evalInfoMask1, 2^25),1);
        mask_tmp.MDH_VOP               = min(bitand(aulEvalInfoMask(2), 2^(53-32)),1); % was 0 in VD
        mask_tmp.MDH_IMASCAN           = ones( Nmeas, 1, 'uint32' );

        noImaScan = ( mask_tmp.MDH_ACQEND           | mask_tmp.MDH_RTFEEDBACK   | mask_tmp.MDH_HPFEEDBACK    ...
                    | mask_tmp.MDH_PHASCOR          | mask_tmp.MDH_NOISEADJSCAN | mask_tmp.MDH_PHASESTABSCAN ...
                    | mask_tmp.MDH_REFPHASESTABSCAN | mask_tmp.MDH_SYNCDATA                              ... 
                    | (mask_tmp.MDH_PATREFSCAN & ~mask_tmp.MDH_PATREFANDIMASCAN) );

        mask_tmp.MDH_IMASCAN( noImaScan ) = 0;
        
        data_uint16_tmp=uint16(mask_tmp.MDH_IMASCAN);
        iIndextmp=find(data_uint16_tmp(:)~=0)-3;
        %data_uint16_tmp=data_uint16_tmp(iIndextmp(1):end);
        data_uint16_tmp=data_uint16(iIndextmp(1):end,5);
        %data_uint16_tmp=data_uint16_tmp.*data_uint16(:,5);
        data_uint16_uniq=unique(data_uint16_tmp);
        
        for iIndex=1:length(data_uint16_uniq)
            data_uint16_tmp(data_uint16_tmp==data_uint16_uniq(iIndex))=iIndex-1;
        end
        %data_uint16(find(data_uint16_tmp~=0),5)=data_uint16_tmp(data_uint16_tmp~=0);
        data_uint16(iIndextmp(1):end,5)=data_uint16_tmp;
        [mdhSMS,maskSMS]=evalMDHCore(data_uint32_SMS,data_uint16_SMS,data_single_SMS,length(rm_row(:)),isVD);
    else
        
        mdhSMS=[];
        maskSMS=[];
    end
    [mdh,mask]=evalMDHCore(data_uint32,data_uint16,data_single,Nmeas,isVD);
    %Add
    
end % of evalMDH()

function [mdh,mask]=evalMDHCore(data_uint32,data_uint16,data_single,Nmeas,isVD)
                                                        %  byte pos.
